					     ...
					     struct fuse_file_info *fp)
{
	...
	fuse_lowlevel_notify_iomap_upsert(ff->fuse, ino, ino, &read, NULL);
}
...
{\n\
	 ...
	 */\n\
	if ((opflags & FUSE_IOMAP_OP_REPORT) && pos <= (16 * %d)) {\n\
		outarg->read.offset = pos;\n\
		outarg->read.length = %d;\n\
		outarg->read.type = ((pos / %d) %% 2) + FUSE_IOMAP_TYPE_MAPPED;\n\
		outarg->read.dev = %d;\n\
		outarg->read.addr = (99 * %d) + pos;\n\
\n\
