...
static void *psi_thread(void *arg)
{
	...
	pthread_setname_np(pthread_self(), psi_shortname(psi->type));
...
void psi_stop_thread(struct psi *psi)
{
	...
	psi->system_fd = -1;
	psi->cgroup_fd = -1;
	psi->thread_running = false;
	pthread_mutex_unlock(&psi->mutex);

	/* Closing the polling fds generates POLLNVAL */
	if (system_fd >= 0)
		close(system_fd);
	if (cgroup_fd >= 0)
		close(cgroup_fd);
	...
	if (psi->thread_running) {
		...
		pthread_cancel(psi->tid);
		pthread_join(psi->tid, NULL);
	}
}
...
void psi_destroy(struct psi **psip)
{
	...
	if (psi) {
		psi_stop_thread(psi);
		pthread_mutex_destroy(&psi->mutex);
		free(psi);
	}
				    ...
				    uint64_t timeout_us)
{
	...
	pthread_mutex_init(&psi->mutex, NULL);
...
int psi_start_thread(struct psi *psi)
{
	...
	psi->thread_running = true;
	error = pthread_create(&psi->tid, NULL, psi_thread, psi);
	if (error) {
