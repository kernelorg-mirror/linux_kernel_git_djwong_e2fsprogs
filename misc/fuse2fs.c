...
#ifdef HAVE_FUSE_LOOPDEV
static void fuse2fs_try_losetup(struct fuse2fs *ff, int flags)
{
	...
	fuse_loopdev_setup(dev_fd, rw ? O_RDWR : O_RDONLY, ff->device, 5,
...
#else
# define fuse2fs_try_losetup(...)	((void)0)
...
int main(int argc, char *argv[])
{
 ...
 _("Allowing users to allocate all blocks. This is dangerous!"));
	...
	if (fctx.iomap_passthrough_options && !fuse2fs_can_iomap(&fctx)) {
		err_printf(&fctx, "%s\n",
			   _("Some mount options require iomap."));
		ret |= 1;
		goto out;
