...
struct fuse2fs {
	...
	uint8_t timing;
#endif
#if defined(HAVE_FUSE_IOMAP) && defined(STATX_WRITE_ATOMIC)
...
{
	return fuse2fs_iomap_does_fileio(ff) &&
			     ...
			     unsigned int *iflags, struct fuse_file_info *fi)
{
	...
	if (fuse2fs_iomap_can_hw_atomic(ff))
		*iflags |= FUSE_IFLAG_IOMAP_ATOMIC;
...
static int fuse2fs_iomap_config_devices(struct fuse_context *ctxt,
					struct fuse2fs *ff)
{
	struct fuse_session *se = fuse_get_session(ctxt->fuse);
	errcode_t err;
			   ...
			   struct fuse_iomap_config *cfg)
{
	...
	ret = fuse2fs_iomap_config_devices(ctxt, ff);
	if (ret)
		goto out_unlock;
