			     ...
			     unsigned int *iflags, struct fuse_file_info *fi)
{
	...
	if (fuse_fs_can_enable_iomap(statbuf, NULL)) {
...
#ifdef HAVE_FUSE_IOMAP
#ifdef STATX_WRITE_ATOMIC
	if (fuse_fs_can_enable_iomap(NULL, stx) &&
...
static int fuse2fs_file_uses_iomap(struct fuse2fs *ff, ext2_ino_t ino)
{
	...
	return fuse_fs_can_enable_iomap(&statbuf, NULL);
}
#else
			  ...
			  struct fuse_file_iomap *write)
{
 ...
 "%s: path=%s nodeid=%llu attr_ino=%llu pos=0x%llx count=0x%llx opflags=0x%x\n",
	 ...
	 */
	if (fuse_file_iomap_can_cache_iomap(opflags) && ff->iomap_cache) {
		...
		fuse_file_iomap_retry_from_cache(read);
	}
...
static int fuse2fs_iomap_config_devices(struct fuse2fs *ff)
{
	...
	dev_index = fuse_fs_iomap_device_add(fd, 0);
	if (dev_index < 0) {
